-- sqldb에서 usertbl 테이블에서 다음 내용들을 확인
USE sqldb;
SELECT * FROM usertbl;
-- usertbl의 인덱스 목록 확인
...
-- usertbl의 상태를 출력하여 인덱스의 내용이 만들어졌는지 확인하고,내용이 없다면 실제 적용되도록 한 후, 인덱스의 크기를 확인
SHOW TABLE STATUS LIKE 'usertbl';

-- 생성한 인덱스를 실제로 적용하기 위해서 ANALYZ로 테이블을 분석하고 처래해줘야함
ANALYZE TABLE usertbl;
-- Index_length가 0이었는데 분석 후 제대로 나오는걸 볼 수 있다. 
SHOW TABLE STATUS LIKE 'usertbl';

-- 출생년도(birthYear)에 보조 인덱스 생성
-- 범수 랑 시경이 생일이 둘다 1979이므로 중복값 때문에 인덱스 생성 에러 발생 
