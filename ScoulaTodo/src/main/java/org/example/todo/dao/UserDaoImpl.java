...
import org.example.todo.command.JDBCUtil;
...
//            next()를 사용해서 resultset의 마지막 행까지 데이터를 가져온다
            while (rs.next()) {
//                결과갑으로 받아온 resultset을 uservo로 변환해준다
                UserVO user = map(rs);
...
//     resultset을 uservo 객체로 메핑(변환)하는 메소드
